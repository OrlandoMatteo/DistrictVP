...
def ruiz(df):
    ...
    if df['G_h'] >= 8 and df['zenith'] <= 85 and df['k_t'] <= 1.2:
    #if df['G_h'] >= 8 and df['k_t'] <= 1.2:
        df['DHI'] = df['k_d']*df['G_h']
        df['DNI'] = (df['G_h']-df['DHI'])/cos(radians(df['zenith']))
    return df
...
def Skartevit1(df):
    ...
    if df['G_h'] >= 8 and df['zenith'] <= 85 and df['k_t'] <= 1.2:
        df['DNI']=df['G_h']*(1-fi)/sin(radians(df['altitude']))
        df['DHI']=df['G_h']-df['DNI']*cos(radians(df['zenith']))
    return df
...
def main(latitude,longitude,elevation,model,df):
    import time
    ...
    loc=pvlib.location.Location(latitude, longitude, tz='Europe/Rome', altitude=elevation, name=None)
    pres=pvlib.atmosphere.alt2pres(loc.altitude)
    #df['azimuth'] = (pvlib.solarposition.get_solarposition(df.index, loc, method='nrel_numba', pressure=pressure, temperature=12)['azimuth']-180).values
    
    #df['altitude']= (pvlib.solarposition.get_solarposition(df.index, loc, method='nrel_numba', pressure=pressure, temperature=12)['elevation']).values
    #df['zenith']=(pvlib.solarposition.get_solarposition(df.index, loc, method='nrel_numba', pressure=pressure, temperature=12)['zenith']).values
    #df['AST']=(pvlib.solarposition.ephemeris(df.index, loc, pressure=pressure, temperature=12)['solar_time']).values
    ...
    df['k_t'] = pvlib.irradiance.disc(df['G_h'], df['zenith'], df.index, pressure=101325)['kt']
